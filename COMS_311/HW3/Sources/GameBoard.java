...
public class GameBoard {
    ...
    private HashMap<HashKey, State> visitedStates; //Holding the visited states
    ...
    public void findAllPaths() {
        ...
        if(initialState.getVehicles().get(0).getLocation()[1] == 18) {
...
//            }
    ...
    public void finishTheGraph() {
        ...
        while (curStates.peek() != null) {
            ...
            if (workingState.level == endState.level - 1) {
                ...
                // NEW LINES //
                hashArray = setVehicleArray(workingState.getVehicles());
                key = new HashKey(hashArray);

                visitedStates.put(key, workingState);
                ...
                for (State workingFutureState : futureStates) {
                    ...
                    if (workingFutureState.getVehicles().get(0).getLocation()[1] == 18) {
    ...
    public boolean helperFindAll(ArrayList<State> futureStates, State parentState) {
        int[] hashArray; // NEW LINE //
        HashKey key; // NEW LINE //
        //For all the future states, check if it's been visited
        for (State workingFutureState : futureStates) {

            // NEW LINE //
            hashArray = setVehicleArray(workingFutureState.getVehicles());
            key = new HashKey(hashArray);
            // NEW LINE //
            ...
            if (!visitedStates.containsKey(key)) {
                ...
                visitedStates.put(key, workingFutureState);
            ...
            if (vehicle.getDirection() == 'h') {
                for (char direction : new char[]{'e', 'w'}) {
                    ...
                    if(validMove(newMove, prevState)) { //If the move is valid
                        ...
                        // NEW CODE //
                        hashArray = setVehicleArray(validState.getVehicles());
                        key = new HashKey(hashArray);

                        if (!visitedStates.containsKey(key)) {
                            //add the new state to nextStates
                            nextStates.add(validState); //old line
                        }
                        // NEW CODE //
    ...
    private boolean tryMove(Pair moveToMake, State curState) {
        ...
        if (moveDir == 'e' || moveDir == 'w') { //Moving left or right (+-1)
            if (moveDir == 'e') { //If its moving right
                ...
                return checkSpace(valueAtWhereVehicleWantsToMove, curState.getVehicles()); //Changed to check space
     ...
     */
    private boolean checkSpace(int spotToCheck, ArrayList<Vehicle> curVehicles) {
        for (Vehicle curVeh : curVehicles) {
